...
int main(int argc, char *argv[])
{
	...
	double		s_min;
	double		s_max;
	double		*u, *uplus, *sigma, *omega, *omega1, *new_sigma;
	double		*sum;
	double		time;
...
int		c, option_index;
	...
	new_sigma = (double *)calloc(n, sizeof(double));
	...
	new_sigma = (double *)calloc(n, sizeof(double));
	if (new_sigma == NULL) {
		printf("Could not allocate memory for \"new_sigma\".\n");
		exit(1);
	}
...
for (i=0;i<n;i++){
	...
	#pragma omp parallel private(it,i) shared(sigma, u,sum) 
	{
	for (it = 0; it < itime; it++) {
		...
		switch (tid)
		   {
		      case 0: //thread 0
			 cblas_dgemv( CblasColMajor, CblasNoTrans, n, n, 1, sigma, n, u, 1, 0, sum, 1);
			 break;
		      case 1: //thread 1
			 cblas_dgemv( CblasColMajor, CblasNoTrans, n, n, 1, sigma, n, u, 1, 0, sum, 1);
			break;
	...
	#pragma omp for
         for (i = 0; i < n; i++) {
			...
			sum[i] -= new_sigma[i]*u[i];
			uplus[i] += dt * sum[i] / divide;
		}
